...
function formatProperty(property: any) {
  ...
  switch (property.type) {
    ...
    case 'select':
      return (
        <span 
          className={`px-2 py-1 rounded text-sm ${colorMap[property.select?.color as keyof typeof colorMap]}`}
        >
    ...
    case 'multi_select':
      return property.multi_select.map((item: any) => (
        <span
          key={item.id}
          className={`mr-1 px-2 py-1 rounded text-sm ${colorMap[item.color as keyof typeof colorMap]}`}
        >
          {item.name}
        </span>
      ));
...
// Add compare function for sorting
...
export function NotionTable({ data }: NotionTableProps) {
  ...
  const sortedAndFilteredPages = data.pages
    .filter((page) => {
      ...
      const description = typedPage.properties[columns.find(col => col.name === 'Description')?.id || ''];
      const descriptionText = description?.type === 'title' 
